...
def sendMsgToAllMgrs(msg):
    ...
    mgrEmails = [manager.uid.email for manager in managers if manager.uid.email != EMAIL_HOST_USER];
    return sendToEmails(msg, mgrEmails);

def sendToEmails(msg, emails):
    # 发送邮件给指定邮箱
    try:
        send_mail("PyToolsIP通知", msg, EMAIL_HOST_USER, emails, fail_silently=False);
        return True;
